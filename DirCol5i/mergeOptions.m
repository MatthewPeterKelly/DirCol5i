...
% - If a field is empty in default, but a struct in user, then the user
% struct is copied exactly.
% - It a field is empty in user, but a struct in default, then the default
% struct is copied exactly.
% - Applies recursively
%
%
    ...
    for i=1:length(default_fields)
        if isfield(user,default_fields{i})
            ...
            elseif C0 && ~C1  %default is struct, user is a field
                if isempty(user.(default_fields{i}))  %Then user wants to copy defaults
                    output.(default_fields{i}) = default.(default_fields{i});
                else
                    disp(['WARNING: ' structName default_fields{i} ' should be a struct!']);
                end
            elseif ~C0 && C1  %default is struct, user is a field
                if isempty(default.(default_fields{i}))  % Then default passes user content directly
                    output.(default_fields{i}) = user.(default_fields{i});
                else
